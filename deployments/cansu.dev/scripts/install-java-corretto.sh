...
for VERSION in $VERSIONS; do
  ...
  dpkg --fsys-tarfile "/tmp/amazon-corretto-${VERSION}-x64-linux-jdk.deb" | tar -xO ./usr/lib/jvm/java-23-amazon-corretto/bin/java > "/tmp/java"
  dpkg --fsys-tarfile "/tmp/amazon-corretto-${VERSION}-x64-linux-jdk.deb" | tar -xO ./usr/lib/jvm/java-23-amazon-corretto/bin/javac > "/tmp/javac"
  dpkg --fsys-tarfile "/tmp/amazon-corretto-${VERSION}-x64-linux-jdk.deb" | tar -xO ./usr/lib/jvm/java-23-amazon-corretto/bin/jar > "/tmp/jar"
